 ...
 * Exported functions:
 * HISTORY:
 * Last edited: Mar 24 22:08 2026 (rd109)
...
typedef struct {
  ...
  Array     loc ;       // for now, the first location this syncmer was seen; of I64
} SyncmerSet ;
...
static inline bool syncmerAdd (SyncmerSet *sms, char *s, I64 *index)
{ bool isAdded = kmerHashAdd (sms->kh, s, index) ;
  if (index)
...
static inline void syncmerCountIncrement (SyncmerSet *sms, I32 sync)
{ if (sync < 0) sync = -sync ;
  ++arr(sms->count,sync,I64) ;
  if (++arr(sms->thisCount,sync,I64) & 0x80) arr(sms->thisCount,sync,I64) = 0x7f ;
}

static inline I64 syncmerCount (SyncmerSet *sms, I32 sync)
{ if (sync < 0) sync = -sync ;
  return arr(sms->count,sync,I64) ;
}

static inline I64 syncmerLoc (SyncmerSet *sms, I32 sync)
{ if (sync < 0) sync = -sync ;
  return arr(sms->loc,sync,I64) ;
}
